...
ip netns exec $netnsleft ip link add dev wg$((${cnt} +1)) numtxqueues $numtxqueues numrxqueues $numrxqueues type wireguard
for ((i=0; i < $numtxqueues; i++)); do
    echo "ffffff,ffffffff,ffffffff" > /sys/class/net/wg$((${cnt} +1))/queues/tx-$i/xps_cpus
    cat /sys/class/net/wg$((${cnt} +1))/queues/tx-$i/xps_cpus
done
for ((i=0; i < $numrxqueues; i++)); do
    echo "ffffff,ffffffff,ffffffff" > /sys/class/net/wg$((${cnt} +1))/queues/rx-$i/rps_cpus
    cat /sys/class/net/wg$((${cnt} +1))/queues/rx-$i/rps_cpus
done

#ip netns exec $netnsleft ip link add dev wg$((${cnt} +1)) type wireguard
#ip netns exec $netnsleft  ./6.26/xdp_redirect_cpu -d wg0 -c 0 -c 1  -S -v -p $xdpprogram &
#pidleft=$!
#ip netns exec $netnsleft taskset --cpu-list 1,2 /home/ulak/aes/boringtune/target/release/boringtun-cli -t 1 -v trace -l /home/ulak/aes/boringtune/wg1_${cnt}.log  wg$((${cnt} +1))
...
ip netns exec $netnsleft ip address add dev wg$((${cnt} +1)) 192.168.$((${cnt} + 1)).$((${cnt} + 1))/24
ip netns exec $netnsleft ip link set up dev wg$((${cnt} +1))
ip netns exec $netnsleft wg set wg$((${cnt} +1)) private-key ./private_left${cnt}
...
ip netns exec $netnsleft ip route add default dev wg$((${cnt} +1))
ip netns exec $netnsleft wg set wg$((${cnt} +1)) listen-port 518$((20 + ${cnt} +1)) peer "$pbl_right" allowed-ips 0.0.0.0/0 endpoint $rightip:518$((20 + ${cnt} +1))
...
ip netns exec $netnsright ip link add dev wg$((${cnt} +1)) numtxqueues $numtxqueues numrxqueues $numrxqueues type wireguard
for ((i=0; i < $numtxqueues; i++)); do
    echo "ffffff,ffffffff,ffffffff" > /sys/class/net/wg$((${cnt} +1))/queues/tx-$i/xps_cpus
done
for ((i=0; i < $numrxqueues; i++)); do
...
#ip netns exec $netnsright ip link add dev wg$((${cnt} +1)) type wireguard
#ip netns exec $netnsright taskset --cpu-list 1,2 /home/ulak/aes/boringtune/target/release/boringtun-cli -t 1 -v trace -l /home/ulak/aes/boringtune/wg2_${cnt}.log  wg$((${cnt} +1))
#ip netns exec $netnsright taskset --cpu-list $((2*${cnt} + 1)) /home/fatihyuce/work/projects/maya3/ng_sdn/wireguard/boringtun/target/release/boringtun-cli --disable-multi-queue --disable-connected-udp   --disable-drop-privileges   -t 1 -v trace -l /home/fatihyuce/work/projects/maya3/ng_sdn/wireguard/boringtun/wg2_${cnt}.log  wg$((${cnt} +1))
ip netns exec $netnsright ip address add dev wg$((${cnt} +1)) 192.168.$((${cnt} + 1)).$((${cnt} + 2))/24
ip netns exec $netnsright ip link set up dev wg$((${cnt} +1))
ip netns exec $netnsright wg set wg$((${cnt} +1)) private-key ./private_right${cnt}
...
ip netns exec $netnsright ip route add default dev wg$((${cnt} +1))
ip netns exec $netnsright wg set wg$((${cnt} +1)) listen-port 518$((20 + ${cnt} +1)) peer "$pbl_left" allowed-ips 0.0.0.0/0 endpoint $leftip:518$((20 + ${cnt} +1))
...
waitiperf $netnsleft 
ip netns exec $netnsright iperf3 -c 10.10.$((${cnt} + 10)).10 -B 10.20.$((${cnt} + 10)).10 -t 1000 -P 128 -M 1310  --forceflush --interval 1   2>&1 | tee output_send_${cnt}.txt >/dev/null 
