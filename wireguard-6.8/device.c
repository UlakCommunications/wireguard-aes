...
static int wg_open(struct net_device *dev)
{
    ...
    if (wg_alloc_percpu_workers(wg) < 0) {
        ret = -ENOMEM;
//        wg_socket_uninit(wg);  // Ensure socket cleanup
        goto out;
    }
	if (dev_v4) {
...
static struct crypto_aead *init_cipher_fallback(bool *is_chacha)
{
    ...
    if (!IS_ERR(tfm)) {
        *is_chacha = false;
        pr_info("Using AES-GCM via kernel crypto API\n");
        return tfm;
    }

    *is_chacha = true;
    pr_warn("AES-GCM unavailable, falling back to built-in ChaCha20-Poly1305\n");
    return NULL;  // ChaCha handled internally, no allocation needed
}
